  ...
  (let* ((data (if (region-active-p)
    ...
    (print
     (format "Molar mass of %s : %.3f g/mol (uma)"   ;; 3 decimal digits
	     data
	     (total-mass (pairs-list elements))))))

(defun total-mass (elem)
...
The function returns pairs of (atoms - elements)"
  (let (p1 ;;first element of a pair (symbol)
    ...
    (while (>= (length elem) 1)
      (cond
       ;; If paren in list, cut-list-in and call recursively.
       ((member "(" elem)
	(setq p2 (cadr (member ")" elem)))
	(setq p1 (total-mass (pairs-list (cut-list-in elem "(" ")"))))
	(setq elem (cut-list-out elem "(" ")")))
	       
       ;; If first is upcase (element) and second is number
       ((and (upcase-p (car elem))
	     (number-p (cadr elem)))
	(setq p1 (cadr (assoc (car elem) elements-mass)))
	(if (not (number-p (caddr elem)))
	    (progn
	      (setq p2 (cadr elem))
	      (setq elem (cddr elem)))
	  ...
	  (progn
       ...
       ((and (upcase-p (car elem))
	     (and (not (upcase-p (cadr elem)))
		  (not (number-p (cadr elem)))))
	(setq p1 (cadr (assoc (concat (car elem) (cadr elem)) elements-mass)))
	(if (number-p (caddr elem))
	    (progn
	      (setq p2 (caddr elem))
	      (setq elem (cdddr elem))
	      ;; If there're two numbers
	      (if (number-p (car elem))
		  (progn
...
(defun cut-list-in ($list $first $last)
...
(defun cut-list-out ($list $first $last)
...
(defconst elements-mass
  "List of elements and their atomic masses"
      '(
	("H" 1.0079)
	("He" 4.0026)
	("Li" 6.941)
